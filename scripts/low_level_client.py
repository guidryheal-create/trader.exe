...
# turn the client to on chain payement for orders and agent
# map the market id and address and resolve the overall system
# make test with cent
# ensure the RSS flux is correct
# filter and prune the result to only usefull information
# follow the masse agent + verification system + early trade
# enhance the overall agent 
...
from core.models.polymarket import SimpleMarket, SimpleEvent
...
class Polymarket:
    def __init__(self) -> None:
        ...
        self.ctf_address = "0x4bFb41d5B3570DeFd03C39a9A4D8dE6Bd8B8982E" # 0x4bFb41d5B3570DeFd03C39a9A4D8dE6Bd8B8982E
    ...
    def get_eth_balance(self) -> float:
        balance_res = self.web3.eth.get_balance(self.get_address_for_private_key())
        return float(balance_res / 1e18)
    
    def get_polymarket_usdc_balance(self, token_id:str) -> dict:
        """
        Returns Polymarket vault balance (free + locked)
        This is NOT wallet USDC.
        """
        
        balances = self.client.get_balance_allowance(BalanceAllowanceParams(asset_type=AssetType.CONDITIONAL, token_id=token_id))
...
if __name__ == "__main__":
    ...
    balance = p.get_eth_balance()
    print("eth", balance)

    print(p.get_polymarket_usdc_balance("11015470973684177829729219287262166995141465048508201953575582100565462316088"))